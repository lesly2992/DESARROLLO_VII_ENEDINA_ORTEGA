...
function calcularCrecimientoVentas($ventas) {
   ...
   foreach ($ventas as $region => $productos) {
       echo "$region:\n";
       foreach ($productos as $producto => $ventasProducto) {
           $ventasInicial = $ventasProducto[0];
           $ventasFinal = end($ventasProducto);
           $crecimiento = (($ventasFinal - $ventasInicial) / $ventasInicial) * 100;
           echo "  $producto: " . number_format($crecimiento, 2) . "%\n";
           // Guardar el mayor crecimiento
...
// Llamada a la función para calcular crecimiento de ventas
$mayorCrecimiento = calcularCrecimientoVentas($ventas);
echo "Producto con mayor crecimiento: {$mayorCrecimiento['producto']} en la región {$mayorCrecimiento['region']} con un crecimiento de " . number_format($mayorCrecimiento['crecimiento'], 2) . "%\n";
